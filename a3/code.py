...
class CustomDistilBert(nn.Module):
    ...
    def forward(self, inputs: transformers.BatchEncoding):
        ...
        cls = self.slice_cls_hidden_state(outputs)
        logits = self.pred_layer(cls)
...
def pad_attention_mask(mask, p):
    ...
    # Pad at the start with 1s because soft prompts are added at the start and should be attended to
    pad = torch.ones(mask.size(0), p, dtype=mask.dtype, device=mask.device)
...
def load_models_and_tokenizer(q_name, a_name, t_name, device='cpu'):
    # TODO: your work below
    q_enc = transformers.AutoModel.from_pretrained(q_name)
    q_enc = q_enc.to(device)
    
    a_enc = transformers.AutoModel.from_pretrained(a_name)
...
def contrastive_loss_criterion(S: Tensor, labels: Tensor = None, device: str = 'cpu'):
    ...
    positive_log_probs = log_probs[torch.arange(N, device=device), labels]
...
def get_topk_indices(Q, P, k: int = None):
    ...
    if k is None:
        indices = torch.argsort(S, dim=1, descending=True)  # (N, M)
        scores = torch.gather(S, 1, indices)
        return indices, scores

    k = min(k, M)
    scores, indices = torch.topk(S, k=k, dim=1, largest=True, sorted=True)
    return indices, scores
...
def select_by_indices(indices: Tensor, passages: 'list[str]') -> 'list[str]':
    ...
    output = []
    for row in indices:
        row_list = [passages[int(idx)] for idx in row.tolist()]
...
def embed_passages(passages: 'list[str]', model, tokenizer, device='cpu', max_length=512):
    ...
    inputs = {k: v.to(model_device) for k, v in batch.items() if isinstance(v, torch.Tensor)}
...
def recall_at_k(retrieved_indices: 'list[list[int]]', true_indices: 'list[int]', k: int):
    ...
    hits = 0
    for i, true in enumerate(true_indices):
...
def mean_reciprocal_rank(retrieved_indices: 'list[list[int]]', true_indices: 'list[int]'):
    ...
    for i, true in enumerate(true_indices):
        ...
        try:
            rank = retrieved.index(true) + 1  # 1-based
            rr_sum += 1.0 / rank
        except ValueError:
            rr_sum += 0.0
    return rr_sum / N
