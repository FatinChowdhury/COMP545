...
def build_current_surrounding_pairs(indices: "list[int]", window_size: int = 2):
    ...
    for i in range(window_size, len(indices)-window_size):
        sources.append(indices[i-window_size:i]+indices[i+1:i+window_size+1])
...
def expand_surrounding_words(ix_surroundings: "list[list[int]]", ix_current: "list[int]"):
    ...
    sources = []
    targets = []
    for target, surroundings in zip(ix_current, ix_surroundings):
        for context in surroundings:
...
def cbow_preprocessing(indices_list: "list[list[int]]", window_size: int = 2):
    ...
    for indices in indices_list:
        sources_inner, targets_inner = build_current_surrounding_pairs(indices, window_size=window_size)
        sources.extend(sources_inner)
        targets.extend(targets_inner)
    return sources, targets
...
def skipgram_preprocessing(indices_list: "list[list[int]]", window_size: int = 2):
    ...
    for indices in indices_list:
        sources_per_center, targets_per_center = build_current_surrounding_pairs(indices, window_size)
        
        for center_word, surrounding_words in zip(targets_per_center, sources_per_center):
            for surrounding_word in surrounding_words:
                all_sources.append(center_word)
                all_targets.append(surrounding_word)
...
def compute_topk_similar(
    ...
    # TODO: your work here
    word_emb_normalized = word_emb / (torch.norm(word_emb) + 1e-8)
    w2v_emb_normalized = w2v_emb_weight / (torch.norm(w2v_emb_weight, dim=1, keepdim=True) + 1e-8)
    cosine_sim = torch.matmul(w2v_emb_normalized, word_emb_normalized.T).squeeze()
    self_idx = torch.argmax(cosine_sim)
    cosine_sim[self_idx] = -float("inf")
    topk_ix = torch.topk(cosine_sim, k=k).indices
...
def retrieve_similar_words(
    ...
    all_emb = model.emb.weight

    topk_ix = compute_topk_similar(word_emb, all_emb, k)
    similar_words = [index_to_word[i] for i in topk_ix]
...
def word_analogy(
    ...
    all_emb = model.emb.weight
    topk_ix = compute_topk_similar(analogy_vec, all_emb, k)
    similar_words = [index_to_word[i] for i in topk_ix]
    return similar_words
...
def compute_gender_subspace(
    ...
    for word1, word2 in gender_attribute_words:
        ...
        # normalizing
        emb1_centered = emb1 - mean
        emb2_centered = emb2 - mean

        normalized_embeddings.extend([emb1_centered, emb2_centered])
    X = np.stack(normalized_embeddings)
...
def compute_profession_embeddings(
    ...
    for profession in professions:
        subwords = profession.split()
        valid_embeddings = [word_to_embedding[w] for w in subwords if w in word_to_embedding]

        if valid_embeddings:
            profession_to_embedding[profession] = np.mean(valid_embeddings, axis=0)
...
def compute_extreme_words(
    ...
    # TODO: your work here
    direction = gender_subspace[0]
    scalars = {}
    for word in words:
        if word in word_to_embedding:
            s, _ = project(word_to_embedding[word], direction)
            scalars[word] = s
    sorted_words = sorted(scalars.items(), key=lambda x: x[1], reverse=max_)
...
def compute_direct_bias(
    ...
    for word in words:
        if word in word_to_embedding:
            similarity = abs(cosine_similarity(word_to_embedding[word], direction)) ** c
            similarity_scores.append(similarity)
    return np.mean(similarity_scores) if similarity_scores else 0.0 # came to this from calculus knowledge
...
def weat_association(
    ...
    mean_A = np.mean([cosine_similarity(emb_w, word_to_embedding[a]) for a in A if a in word_to_embedding])
    mean_B = np.mean([cosine_similarity(emb_w, word_to_embedding[b]) for b in B if b in word_to_embedding])
...
def weat_differential_association(
    ...
    # TODO: your work here
    s_X = np.sum([weat_association(x, A, B, word_to_embedding) for x in X if x in word_to_embedding])
    s_Y = np.sum([weat_association(y, A, B, word_to_embedding) for y in Y if y in word_to_embedding])
    return s_X - s_Y
...
def debias_word_embedding(
    ...
    direction = gender_subspace[0]
    scalar, proj = project(emb, direction)
    emb-=proj
    return emb
...
def hard_debias(
    ...
    # TODO: your work here
    # identify gender subspace first
